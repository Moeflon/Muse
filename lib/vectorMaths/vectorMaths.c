...
Vector32 vector_to_vector32(Vector* v) {
  Vector32 v32 = {v->x, v->y, v->z};
  return v32;
}

Vector vector32_to_vector(Vector* v32) {
  Vector v = {v32->x, v32->y, v32->z};
  return v;
}
...
void coord_transform(Vector* measurement, int16_t m_scale,  Vector* orientation, int16_t o_scale) {
    int16_t x = measurement->x / m_scale;
    int16_t y = measurement->y / m_scale;
    int16_t z = measurement->z / m_scale;

    int16_t alpha = orientation->x / o_scale;
    int16_t beta  = orientation->y / o_scale;
    int16_t gamma = orientation->z / o_scale;

    int16_t cos_a = cos100(alpha);
    int16_t cos_b = cos100(beta);
    int16_t cos_g = cos100(gamma);
    int16_t sin_a = sin100(alpha);
    int16_t sin_b = sin100(beta);
    int16_t sin_g = sin100(gamma);

    int16_t sin_a_sin_b = (sin_a * sin_b);
    int16_t cos_a_sin_b = (cos_a * sin_b);
    int16_t sin_a_cos_b = (sin_a * cos_b);
    int16_t cos_a_cos_b = (cos_a * cos_b);

    int32_t new_x = (cos_g * (((sin_a_sin_b*y - cos_a_sin_b*z) / 100) + cos_b*x)) + (sin_g * (sin_a*z + cos_a*y));
    int32_t new_y = (sin_g * (((-sin_a_sin_b*y + cos_a_sin_b*z) / 100)  - cos_b*x)) + (cos_g * (sin_a*z + cos_a*y));
    int32_t new_z = sin_b * x - ((sin_a_cos_b * y + cos_a_cos_b * z) / 100);

    measurement->x = (new_x / 10000) * m_scale;
    measurement->y = (new_y / 10000) * m_scale;
    measurement->z = (new_z / 100) * m_scale;
}
